...
def _prepare_app_usgae_features(app_usage_file):
    """
    ...
    # Merge features
    features = features.merge(time_diff_features, on='member_id')
    features = features.merge(last_visit_time[['member_id', 'time_from_last_visit']], on='member_id')

    return features

# Example usage
# features_df = prepare_features('app_usage.csv')
# print(features_df.head())

def _prepare_churn_label_features(churn_file):
    """
    ...
    """
    # Load the data
    churn_labels = pd.read_csv(churn_file)
    ...
    # Calculate time from current time to last visit
    current_time = datetime.now()
    churn_labels['days_from_signup'] = (current_time - churn_labels['signup_date']).dt.days

    return churn_labels


def _prepare_web_visit_features(web_visits_file):
    """
    ...
    features = features.merge(visit_pattern, on='member_id', how='left')
    ...
    features = features.merge(mode_time_of_day, on='member_id', how='left')
    ...
    features = features.merge(time_diff_features, on='member_id', how='left')

    return features

# Example usage
# features_df = prepare_web_visit_features('web_visits.csv')
# print(features_df.head())

def _prepare_claims_features(claims_file, collapse_icd=True, icd_keep_list=None):
    """
    ...
    features = features.merge(data[['member_id', 'num_diff_diagnoses']].drop_duplicates(), on='member_id', how='left')
...
def run(data_folder):
    ...
    churn_file = os.path.join(data_folder, 'churn.csv')

    # Prepare features
    print("Preparing app usage features...")
    app_usage_features = _prepare_app_usgae_features(app_usage_file)

    print("Preparing web visit features...")
    web_visit_features = _prepare_web_visit_features(web_visits_file)

    print("Preparing claims features...")
    claims_features = _prepare_claims_features(claims_file)

    print("Preparing churn label features...")
    churn_labels = _prepare_churn_label_features(churn_file)

    # Merge all DataFrames on member_id
    print("Merging all features into one dataset...")
    final_dataset = app_usage_features.merge(web_visit_features, on='member_id', how='outer')
    final_dataset = final_dataset.merge(claims_features, on='member_id', how='outer')
    final_dataset = final_dataset.merge(churn_labels, on='member_id', how='outer')
